...
#starting reference point for the tags in jobs loop
    def parse(self, response):
        ...
        for job in jobs:
            ...
            item['title'] = title0 + ("" + title1 if title1 else "")
            item['company'] = job.xpath('span/span/a/text()').extract_first()
            item['location'] = job.xpath('span[@itemprop="jobLocation"]/span/span/text()').extract_first("")
            relative_url = job.xpath('h2/a/@href').extract_first()
            item['link'] = response.urljoin(relative_url)
